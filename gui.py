...
class BattleshipGUI:
    def __init__(self, master):
        ...
        self.ai_canvas.grid(row=0, column=0)
        self.human_canvas.grid(row=1, column=0)

        self.ai_canvas.bind("<Button-1>", self.ai_canvas_click)
        self.human_canvas.bind("<Button-1>", self.human_canvas_click)

        self.feedback_text = tk.Text(master, wrap=tk.WORD, height=20, width=50)
        self.feedback_text.grid(row=0, column=1, rowspan=2)
    ...
    def human_canvas_click(self, event):
        print("Clicked on human_canvas")
        if self.stage == 1:
            x, y = event.y // 30, event.x // 30
            coord = f"{chr(65+y)}-{x+1}"
            if self.start_x is None and self.start_y is None:
                self.start_x, self.start_y = x, y
                self.feedback_text.insert(tk.END, f"Ship started to be placed at {coord}\n")
            else:
                ...
                if self.game.is_valid_human_ship_position(ship, self.start_x, self.start_y, orientation):
                    ...
                    self.feedback_text.insert(tk.END, f"Ship placement confirmed at {coord}\n")
                    self.ship_index += 1
                    ...
                    self.feedback_text.insert(tk.END, f"Ship placement at {coord} invalid - Resetting\n")
                self.start_x, self.start_y = None, None
    ...
    def ai_canvas_click(self, event):
        if self.stage == 2:
            ...
            if hit:
                self.ai_canvas.create_rectangle(y * 30, x * 30, y * 30 + 30, x * 30 + 30, fill="red")
                self.feedback_text.insert(tk.END, f"Human Fired at {coord} - HIT\n")
            else:
                self.ai_canvas.create_oval(y * 30 + 10, x * 30 + 10, y * 30 + 20, x * 30 + 20, fill="blue")
                self.feedback_text.insert(tk.END, f"Human Fired at {coord} - MISS\n")
            if self.game.game_over():
                ...
                coord = f"{chr(65+y)}-{x+1}"
                if hit:
                    self.human_canvas.create_rectangle(y * 30, x * 30, y * 30 + 30, x * 30 + 30, fill="red")
                    self.feedback_text.insert(tk.END, f"AI Fired at {coord} - HIT\n")
                else:
                    self.human_canvas.create_oval(y * 30 + 10, x * 30 + 10, y * 30 + 20, x * 30 + 20, fill="blue")
                    self.feedback_text.insert(tk.END, f"AI Fired at {coord} - MISS\n")
                if self.game.game_over():
                    self.stage = 3
                    # Option to play again can be added here
